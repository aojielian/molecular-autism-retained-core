...
cfg <- list(
  ...
  outdir = "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/MolecularAutism_Revision/Figure3_Discovery_DiseaseAssociation_v2",
...
safe_read <- function(path) {
  ...
  if (grepl("\\.gz$", path)) fread(cmd = paste("gzip -dc", shQuote(path)), sep = "\t", header = TRUE, data.table = TRUE) else fread(path, sep = "\t", header = TRUE, data.table = TRUE)
}
...
plot_box_metric <- function(dt, y, ylab, title, stat_row) {
  ann <- paste0(
    "effect = ", sprintf("%.3f", stat_row$effect_case_minus_control),
  ...
  ggplot(dt, aes(x = dx, y = .data[[y]], fill = dx)) +
    ...
    annotate(
      "label",
      x = 1.5,
      y = max(dt[[y]], na.rm = TRUE) + 0.04 * diff(range(dt[[y]], na.rm = TRUE)),
      label = ann,
      size = 3.3,
      hjust = 0.5,
      vjust = 1.1,
...
# label top nominal genes only when nothing passes FDR<0.10
n_label <- 10L
if (n_fdr01 == 0L) {
...
summary_dt <- data.table(
  ...
  finding = c(
    paste0(
      ...
      "\nEffect = ", sprintf("%.3f", stat_prop$effect_case_minus_control),
      "; lm P = ", p_fmt(stat_prop$lm_p)
    ),
    paste0(
      ifelse(stat_ratio$effect_case_minus_control > 0, "Higher in ASD", "Lower in ASD"),
      "\nEffect = ", sprintf("%.3f", stat_ratio$effect_case_minus_control),
  ...
  row = c(3, 2, 1),
  fill = c(cfg$asd_color, cfg$asd_color, "#E6E6E6")
)
summary_dt[, fill_alpha := c(0.22, 0.22, 1.0)]

pD <- ggplot(summary_dt) +
  geom_tile(aes(x = 1, y = row), width = 1.90, height = 0.86,
            fill = alpha(summary_dt$fill, summary_dt$fill_alpha), color = "black", linewidth = 0.35) +
  geom_text(aes(x = 0.38, y = row, label = endpoint), hjust = 0, vjust = 0.5,
            size = 4.0, fontface = "bold") +
  geom_text(aes(x = 1.10, y = row, label = finding), hjust = 0, vjust = 0.5,
            size = 3.55) +
  xlim(0, 2.02) +
  ylim(0.5, 3.5) +
