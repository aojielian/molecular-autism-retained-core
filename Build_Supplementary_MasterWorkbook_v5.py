...
def derive_subject_dx_counts_from_meta(
    ...
    counts: Dict[str, int] = {}
    for _, rec in subjects.items():
        dx = normalize_str(rec.get('diagnosis'))
        if dx == '':
            continue
        if allowed_dx and dx not in allowed_dx:
            continue
        counts[dx] = counts.get(dx, 0) + 1
    return counts


def derive_qc_subjects_from_meta(
    meta_rows: List[Dict[str, str]],
    mapping: Dict[str, str],
    allowed_subjects: Optional[Set[str]] = None,
    allowed_samples: Optional[Set[str]] = None,
...
def derive_qc_samples_from_meta(
    meta_rows: List[Dict[str, str]],
    mapping: Dict[str, str],
    allowed_subjects: Optional[Set[str]] = None,
    allowed_samples: Optional[Set[str]] = None,
    ...
    for r in rows:
        if dx_col:
            dx = canonical_dx(r.get(dx_col))
            if allowed_dx and dx not in allowed_dx:
                continue
        s = normalize_str(r.get(sample_col)) if sample_col else ''
        if s:
            seen.add(s)
    return len(seen)
...
def summarize_sex_distribution(values: List[str]) -> str:
    counts: Dict[str, int] = {}
    for v in values:
        v = canonical_sex(v)
        if v == '':
    ...
    parts = []
    if 'F' in counts:
        parts.append(f"F={counts['F']}")
    if 'M' in counts:
        parts.append(f"M={counts['M']}")
    for k in sorted(counts.keys()):
        if k in {'F', 'M'}:
            continue
        if counts[k] == 1:
            parts.append(f'one donor with {k} karyotype')
        else:
            parts.append(f'{counts[k]} donors with {k} karyotype')
    return '; '.join(parts)
...
def cohort_context(
    ...
    return {
        ...
        'sex_distribution': summarize_sex_distribution(sex_vals),
...
val_allowed_dx = {'ASD', 'Control'}

if val_counts['n_ASD'] is None and val_counts['n_Control'] is None:
    inferred = derive_subject_dx_counts_from_meta(
        validation_meta_rows,
        COHORT_META['Velmeshev validation'],
        allowed_subjects=val_shared_subjects or None,
        allowed_samples=val_shared_samples or None,
        allowed_dx=val_allowed_dx,
    )
    if inferred:
        val_counts = {
            'n_ASD': inferred.get('ASD', 0),
            'n_Control': inferred.get('Control', 0),
        }

val_qc_subjects = derive_qc_subjects_from_meta(
    validation_meta_rows,
    COHORT_META['Velmeshev validation'],
    allowed_subjects=val_shared_subjects or None,
    allowed_samples=val_shared_samples or None,
...
if val_counts['n_ASD'] is not None or val_counts['n_Control'] is not None or val_qc_subjects > 0:
    n_asd_cases = val_counts['n_ASD'] if val_counts['n_ASD'] is not None else ''
    n_ctl_cases = val_counts['n_Control'] if val_counts['n_Control'] is not None else ''
    n_total_cases = ''
    ...
    s1.append(finalize_s1_row({
        ...
        'n_ASD_cases': n_asd_cases,
        'n_Control_cases': n_ctl_cases,
        'n_total_cases': n_total_cases,
        'qc_retained_subjects': val_qc_subjects if val_qc_subjects > 0 else n_total_cases,
        'qc_retained_nuclei': '',
        'qc_retained_samples': derive_qc_samples_from_meta(
            validation_meta_rows,
            COHORT_META['Velmeshev validation'],
            allowed_subjects=val_shared_subjects or None,
            allowed_samples=val_shared_samples or None,
        ...
        'notes': note,
    }, 'Velmeshev validation', validation_meta_rows, COHORT_META['Velmeshev validation'], allowed_subjects=val_shared_subjects or None, allowed_samples=val_shared_samples or None, allowed_dx=val_allowed_dx))
...
if val_micro and {'dx', 'N'} <= set(val_micro[0].keys()):
    ...
    s1.append(finalize_s1_row({
        ...
        'n_Control_cases': '',
        'n_total_cases': '',
        'qc_retained_subjects': derive_qc_subjects_from_meta(
            validation_meta_rows,
            COHORT_META['Velmeshev validation'],
            allowed_subjects=val_shared_subjects or None,
            allowed_dx={'ASD', 'Control'},
        ) or '',
...
if rows_gandal_dx and {'dx', 'N'} <= set(rows_gandal_dx[0].keys()):
    ...
    inferred = derive_subject_dx_counts_from_meta(
        gandal_meta_rows,
        COHORT_META['Gandal 2022 bulk cortex'],
        allowed_subjects=gandal_filtered_subjects or None,
        allowed_samples=gandal_filtered_samples or None,
        allowed_dx=gandal_allowed_dx,
    )
    if inferred:
        n_asd = inferred.get('ASD', 0)
        n_ctl = inferred.get('Control', 0)

if (n_asd is not None and n_ctl is not None) or gandal_filtered_samples or gandal_filtered_subjects:
    ...
    gandal_qc_subjects = derive_qc_subjects_from_meta(
        gandal_meta_rows,
        COHORT_META['Gandal 2022 bulk cortex'],
        allowed_subjects=gandal_filtered_subjects or None,
        allowed_samples=gandal_filtered_samples or None,
    ...
    gandal_qc_samples = derive_qc_samples_from_meta(
        gandal_meta_rows,
        COHORT_META['Gandal 2022 bulk cortex'],
        allowed_subjects=gandal_filtered_subjects or None,
        allowed_samples=gandal_filtered_samples or None,
    ...
    s1.append(finalize_s1_row({
        ...
        'n_ASD_cases': n_asd if n_asd is not None else '',
        'n_Control_cases': n_ctl if n_ctl is not None else '',
        'n_total_cases': (n_asd or 0) + (n_ctl or 0) if (n_asd is not None or n_ctl is not None) else '',
        'qc_retained_subjects': gandal_qc_subjects or (len(gandal_filtered_subjects) if gandal_filtered_subjects else ''),
        'qc_retained_nuclei': '',
        'qc_retained_samples': gandal_qc_samples or (len(gandal_filtered_samples) if gandal_filtered_samples else ''),
        'notes': note,
    }, 'Gandal 2022 bulk cortex', gandal_meta_rows, COHORT_META['Gandal 2022 bulk cortex'], allowed_subjects=gandal_filtered_subjects or None, allowed_samples=gandal_filtered_samples or None, allowed_dx=gandal_allowed_dx))

# Ensure stable reviewer-facing ordering for the minimum five expected S1 rows
s1_order = {
    ('Discovery', 'Discovery audited microglial object'): 1,
    ('Discovery', 'Discovery retained-core object'): 2,
    ('Velmeshev validation', 'Strict shared validation set'): 3,
    ('Velmeshev validation', 'Validation microglia retained for descriptive context'): 4,
    ('Gandal 2022 bulk cortex', 'Filtered external bulk-cortex cohort'): 5,
}
s1 = clean_rows(s1)
s1 = sorted(s1, key=lambda r: (s1_order.get((normalize_str(r.get('dataset')), normalize_str(r.get('summary_scope'))), 999), normalize_str(r.get('dataset')), normalize_str(r.get('summary_scope'))))
