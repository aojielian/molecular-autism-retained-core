...
def main():
    ap = argparse.ArgumentParser(description="Local perturbation-based prioritization for the candidate/reference remodeling axis")
    ap.add_argument(
        "--signature_tsv",
        default="/Users/aojie/PROJECTs/molecular_autism/drug/08_discovery_frozen_state_signature.tsv"
    )
    ap.add_argument(
        "--pooled_de_tsv",
        default="/Users/aojie/PROJECTs/molecular_autism/drug/13_pseudobulk_all_retained_ASD_vs_Control.tsv.gz"
    )
    ap.add_argument(
        "--outdir",
        default="/Users/aojie/PROJECTs/molecular_autism/drug/Package9_PerturbationPrioritization_local"
    )
    ...
    with open(os.path.join(args.outdir, "tables", "06_sensitivity_pooled_up_genes.txt"), "w") as f:
        f.write("\n".join(up2) + "\n")
    with open(os.path.join(args.outdir, "tables", "07_sensitivity_pooled_down_genes.txt"), "w") as f:
        f.write("\n".join(down2) + "\n")
    ...
    write_tsv(sens_geo_down, os.path.join(args.outdir, "tables", "08_sensitivity_pooled_up_vs_GEO_drug_down.tsv"))
    ...
    write_tsv(sens_geo_up, os.path.join(args.outdir, "tables", "09_sensitivity_pooled_down_vs_GEO_drug_up.tsv"))

    sens_combined = combine_reversal(sens_geo_down, sens_geo_up, top_n=200)
    write_tsv(sens_combined, os.path.join(args.outdir, "tables", "10_sensitivity_reversal_combined.tsv"))
    ...
    write_tsv(overlap_df, os.path.join(args.outdir, "tables", "11_primary_vs_sensitivity_overlap.tsv"))
    ...
    summarise_run(
        primary_combined,
        sens_combined,
        os.path.join(args.outdir, "12_summary.txt")
    )
    ...
    with open(os.path.join(args.outdir, "13_run_parameters.json"), "w") as f:
