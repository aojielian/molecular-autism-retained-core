...
cfg <- list(
  pkg8  = "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/MolecularAutism_Revision/Package8_ExternalValidation_Gandal2022",
  pkg8b = "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/MolecularAutism_Revision/Package8b_Gandal2022_LOO_Sensitivity",
  outdir = "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/MolecularAutism_Revision/Figure6_Gandal2022_ExternalValidation_v1"
)
...
# ------------------------------
dx_col <- find_col(scores, c("dx", "Diagnosis", "diagnosis", "group", "Group", "Dx"))
region_col <- find_col(scores, c("region", "Region", "cortical_region", "brain_region"), required = FALSE)
...
res_beta_col <- find_col(results, c("beta", "estimate", "coef", "effect", "dx_beta", "Estimate"))
res_p_col <- find_col(results, c("p_value", "p", "pval", "PValue", "Pr(>|t|)", "pvalue"))
res_fdr_col <- find_col(results, c("fdr", "FDR", "padj", "adj_p", "q_value", "qvalue"), required = FALSE)
...
loo_beta_col <- find_col(loo, c("beta", "estimate", "coef", "effect", "dx_beta", "Estimate"))
loo_region_col <- find_col(loo, c("left_out_region", "leave_out_region", "excluded_region", "region", "held_out_region"))
loo_p_col <- find_col(loo, c("p_value", "p", "pval", "PValue", "pvalue"), required = FALSE)
loo_fdr_col <- find_col(loo, c("fdr", "FDR", "padj", "adj_p", "q_value"), required = FALSE)
...
pD <- make_loo_panel("candidate_program", "D. Leave-one-region-out candidate-program stability")
pE <- make_loo_panel("reference_program", "E. Leave-one-region-out reference-program stability")
pF <- make_loo_panel("candidate_minus_reference", "F. Leave-one-region-out candidate-minus-reference stability")
