...
plot_box_metric <- function(dt, y, ylab, title, stat_row) {
  ann <- paste0(
    "ASD-Control β = ", sprintf("%.3f", stat_row$effect_case_minus_control),
...
summary_dt <- data.table(
  ...
  finding = c(
    paste0(
      ...
      "\nASD-Control β = ", sprintf("%.3f", stat_prop$effect_case_minus_control),
      "; lm P = ", p_fmt(stat_prop$lm_p)
    ),
    paste0(
      ifelse(stat_ratio$effect_case_minus_control > 0, "Higher in ASD", "Lower in ASD"),
      "\nASD-Control β = ", sprintf("%.3f", stat_ratio$effect_case_minus_control),
