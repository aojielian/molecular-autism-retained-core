...
OUTDIR = ROOT / 'Supplementary_Tables_v3'
OUTFILE = OUTDIR / 'Supplementary_Tables_Master_v3.xlsx'
...
if exact_n_rows:
    ...
    for r in exact_n_rows:
        ...
        grouped[scope].setdefault(diagnosis, {})
        grouped[scope][diagnosis]['n_cells'] = maybe_int(r.get('n_cells'))
        grouped[scope][diagnosis]['n_donors'] = maybe_int(r.get('n_donors'))
    ...
    for scope, diag_map in grouped.items():
        ...
        s1.append({
            ...
            'n_ASD': asd_map.get('n_donors', ''),
            'n_Control': ctl_map.get('n_donors', ''),
            'n_total': all_map.get('n_donors', ''),
            'notes': f"Derived from {'A/B' if scope == 'Discovery audited microglial object' else 'D/E'} exact n summary."
        })
        s1.append({
            'dataset': 'Discovery',
            'summary_scope': scope,
            'analysis_unit': 'cells',
            'n_ASD': asd_map.get('n_cells', ''),
            'n_Control': ctl_map.get('n_cells', ''),
            'n_total': all_map.get('n_cells', ''),
            'notes': f"Derived from {'A/B' if scope == 'Discovery audited microglial object' else 'D/E'} exact n summary."
        })
else:
