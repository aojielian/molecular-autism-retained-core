...
def main():
    ...
    ap.add_argument(
        "--indir",
        default=env_or_none("MA_DRUG_TABLES_DIR") or project_default("results", "Package9_PerturbationPrioritization", "tables")
    )
    ap.add_argument(
        "--outdir",
        default=env_or_none("MA_DRUG_POST_OUTDIR") or project_default("results", "Package9_PerturbationPrioritization", "postprocessed")
    )
    ...
    if args.indir is None or args.outdir is None:
        ap.error("Provide --indir and --outdir, or set MA_PROJECT_DIR/MA_DRUG_TABLES_DIR/MA_DRUG_POST_OUTDIR")
