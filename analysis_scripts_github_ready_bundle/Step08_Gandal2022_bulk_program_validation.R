...
if (length(args) >= 4) {
  ...
  rdata_path     <- Sys.getenv("MA_GANDAL_RDATA", unset = "")
  candidate_path <- Sys.getenv("MA_CANDIDATE_GENES", unset = "")
  reference_path <- Sys.getenv("MA_REFERENCE_GENES", unset = "")
  outdir         <- Sys.getenv("MA_GANDAL_OUTDIR", unset = "")

  if (rdata_path == "" || candidate_path == "" || reference_path == "" || outdir == "") {
    stop(paste(
      "Usage: Rscript Step08_Gandal2022_bulk_program_validation_github_ready.R <RData> <candidate_genes.txt> <reference_genes.txt> <outdir>",
      sprintf("or set environment variables MA_GANDAL_RDATA, MA_CANDIDATE_GENES, MA_REFERENCE_GENES, and MA_GANDAL_OUTDIR"),
      sep = "
"
    ))
  }
