...
option_list <- list(
  ...
  make_option("--pkg2_outdir", type = "character", default = file.path(project_dir_default, "results", "Package2_Define_Cluster2"), help = "Package2 output directory [default: %default]"),
  make_option("--pkg3_outdir", type = "character", default = file.path(project_dir_default, "results", "Package3_Discovery_DonorAwareDiseaseAssociation"), help = "Package3 output directory [default: %default]"),
  make_option("--outdir", type = "character", default = file.path(project_dir_default, "results", "Figure2_RetainedCore_DonorLevel"), help = "Output directory [default: %default]")
)
opt <- parse_args(OptionParser(option_list = option_list))

cfg <- list(
  strict_rds = opt$strict_rds,
  pkg2_outdir = opt$pkg2_outdir,
  pkg3_outdir = opt$pkg3_outdir,
  pkg2_cluster2_summary = file.path(opt$pkg2_outdir, "tables", "22_cluster2_core_summary_up.tsv"),
  pkg2_cluster0_summary = file.path(opt$pkg2_outdir, "tables", "23_cluster0_core_summary_up.tsv"),
  pkg2_cell_de = file.path(opt$pkg2_outdir, "tables", "06_celllevel_DE_cluster2_vs_0.tsv.gz"),
  pkg3_abundance = file.path(opt$pkg3_outdir, "02_abundance_per_donor.tsv"),
  pkg3_abundance_stats = file.path(opt$pkg3_outdir, "03_abundance_stats.tsv"),
  pkg3_top_state_genes = file.path(opt$pkg3_outdir, "08_state_definition_top_genes.tsv"),
  pkg3_oof_scores = file.path(opt$pkg3_outdir, "09_oof_state_concordance_per_donor.tsv"),
  pkg3_oof_stats = file.path(opt$pkg3_outdir, "10_oof_state_concordance_stats.tsv"),
  outdir = opt$outdir,
  assay = "RNA",
