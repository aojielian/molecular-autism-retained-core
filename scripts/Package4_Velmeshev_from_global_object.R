...
option_list <- list(
  ...
  make_option("--val_global_cluster_col", type = "character", default = "AUTO"),
...
resolve_column <- function(meta, user_col = "AUTO", type = c("cluster", "donor", "dx")) {
  ...
  candidates <- switch(
    ...
    donor = c(
      "donor_id", "donor", "individualID", "individual_ID", "individual_id",
      "subject_id", "subject", "projid", "sample_id", "sample", "patient",
      "Patient", "donorID", "Donor"
    ),
    dx = c(
      "Diagnosis", "diagnosis", "Dx", "dx", "Group", "group", "condition",
      "Condition", "phenotype", "Phenotype", "disease", "Disease"
    )
  )
...
infer_donor_from_metadata_or_cellnames <- function(meta) {
  n_cells <- nrow(meta)
  nms <- colnames(meta)
  ...
  donor_candidates <- c(
    "donor_id", "donor", "individualID", "individual_ID", "individual_id",
    "subject_id", "subject", "projid", "sample_id", "sample", "patient",
    "Patient", "donorID", "Donor", "library", "sample_name", "Sample",
    "sampleID", "batch", "orig.ident"
  )

  candidate_hits <- donor_candidates[donor_candidates %in% nms]
  if (length(candidate_hits) > 0) {
    stats <- rbindlist(lapply(candidate_hits, function(nm) {
      u <- unique(as.character(meta[[nm]]))
      u <- u[!is.na(u) & u != ""]
      data.table(column = nm, n_unique = length(u), examples = paste(head(u, 8), collapse = " | "))
    }))
    stats <- stats[n_unique >= 5 & n_unique < n_cells]
    if (nrow(stats) > 0) {
      best <- stats[order(abs(n_unique - 50))]$column[1]
      return(list(source = "metadata", column = best, values = as.character(meta[[best]]), summary = stats))
    }
  }

  cellnames <- meta$cell

  parse_prefix <- function(x, sep) {
    sapply(strsplit(x, sep, fixed = TRUE), `[`, 1)
  }

  parse_candidates <- list(
    cell_prefix_underscore = parse_prefix(cellnames, "_"),
    cell_prefix_colon = parse_prefix(cellnames, ":"),
    cell_prefix_pipe = parse_prefix(cellnames, "|"),
    cell_prefix_dash = parse_prefix(cellnames, "-")
  )

  parse_stats <- rbindlist(lapply(names(parse_candidates), function(nm) {
    vals <- parse_candidates[[nm]]
    u <- unique(vals)
    u <- u[!is.na(u) & u != ""]
    data.table(source = nm, n_unique = length(u), examples = paste(head(u, 8), collapse = " | "))
  }))
  parse_stats <- parse_stats[n_unique >= 5 & n_unique < n_cells]

  if (nrow(parse_stats) > 0) {
    best_source <- parse_stats[order(abs(n_unique - 50))]$source[1]
    return(list(source = best_source, column = best_source, values = parse_candidates[[best_source]], summary = parse_stats))
  }

  stop("无法识别 validation donor 列，也无法从 cell names 解析 donor。")
}
...
log_msg("Reading validation global object: %s", opt$val_rds)
...
val_global_cluster_col <- resolve_column(val_meta0, opt$val_global_cluster_col, "cluster")
val_dx_col             <- resolve_column(val_meta0, opt$val_dx_col, "dx")

if (!identical(opt$val_donor_col, "AUTO")) {
  if (!opt$val_donor_col %in% colnames(val_meta0)) stop("指定的 validation donor 列不存在")
  val_donor_info <- list(
    source = "metadata_manual",
    column = opt$val_donor_col,
    values = as.character(val_meta0[[opt$val_donor_col]]),
    summary = data.table(column = opt$val_donor_col, n_unique = uniqueN(val_meta0[[opt$val_donor_col]]))
  )
} else {
  val_donor_info <- infer_donor_from_metadata_or_cellnames(val_meta0)
}
...
val_meta0[, global_cluster := as.character(get(val_global_cluster_col))]
val_meta0[, dx := normalize_dx(get(val_dx_col), opt$case_label, opt$control_label)]
val_meta0[, donor := as.character(val_donor_info$values)]

disc_keep <- c(opt$disc_cluster0, opt$disc_cluster2)
disc_meta <- disc_meta[cluster %in% disc_keep & !is.na(donor) & donor != "" & !is.na(dx)]
val_meta0 <- val_meta0[!is.na(donor) & donor != "" & !is.na(dx) & !is.na(global_cluster) & global_cluster != ""]
...
safe_fwrite(
  data.table(
    ...
    val_global_cluster_col = val_global_cluster_col,
    val_dx_col = val_dx_col,
    inferred_val_donor_source = val_donor_info$source,
    inferred_val_donor_column = val_donor_info$column,
    disc_n_cells = nrow(disc_meta),
    disc_n_donors = uniqueN(disc_meta$donor),
    val_global_n_cells = nrow(val_meta0),
    val_global_n_donors = uniqueN(val_meta0$donor),
    val_global_n_clusters = uniqueN(val_meta0$global_cluster)
  ),
  file.path(opt$outdir, "01_run_parameters.tsv")
)

safe_fwrite(val_donor_info$summary, file.path(opt$outdir, "02_inferred_validation_donor_summary.tsv"))

log_msg("Subsetting validation cells where %s == %s", val_global_cluster_col, opt$val_microglia_label)
val_meta_mg <- val_meta0[global_cluster == opt$val_microglia_label]
if (nrow(val_meta_mg) == 0) stop(sprintf("在 validation object 中没有找到 global cluster == %s", opt$val_microglia_label))

val_obj_mg <- subset(val_obj, cells = val_meta_mg$cell)
val_meta_mg <- as.data.table(val_obj_mg@meta.data, keep.rownames = "cell")
val_meta_mg[, dx := normalize_dx(get(val_dx_col), opt$case_label, opt$control_label)]
val_meta_mg[, donor := as.character(val_donor_info$values[match(cell, val_meta0$cell)])]

safe_fwrite(val_meta_mg[, .(cell, donor, dx)], file.path(opt$outdir, "03_validation_microglia_cell_manifest.tsv.gz"))

log_msg("Reclustering validation microglia")
DefaultAssay(val_obj_mg) <- val_assay
val_obj_mg <- NormalizeData(val_obj_mg, assay = val_assay, verbose = FALSE)
val_obj_mg <- FindVariableFeatures(val_obj_mg, assay = val_assay, selection.method = "vst", nfeatures = 2000, verbose = FALSE)
val_obj_mg <- ScaleData(val_obj_mg, assay = val_assay, verbose = FALSE)
val_obj_mg <- RunPCA(val_obj_mg, assay = val_assay, npcs = max(opt$val_npcs, 20), verbose = FALSE)
...
val_obj_mg <- RunUMAP(val_obj_mg, dims = 1:opt$val_npcs, verbose = FALSE)

val_meta <- as.data.table(val_obj_mg@meta.data, keep.rownames = "cell")
val_meta[, donor := as.character(val_donor_info$values[match(cell, val_meta0$cell)])]
...
safe_fwrite(val_meta[, .(cell, donor, dx, val_subcluster)], file.path(opt$outdir, "04_validation_microglia_reclustered_manifest.tsv.gz"))
saveRDS(val_obj_mg, file.path(opt$outdir, "05_validation_microglia_reclustered_object.rds"))

umap_df <- as.data.table(Embeddings(val_obj_mg, "umap"), keep.rownames = "cell")
setnames(umap_df, c("cell", "UMAP_1", "UMAP_2"))
umap_df <- merge(umap_df, val_meta[, .(cell, dx, val_subcluster)], by = "cell", all.x = TRUE)
safe_fwrite(umap_df, file.path(opt$outdir, "06_validation_microglia_umap.tsv.gz"))

p_umap <- ggplot(umap_df, aes(x = UMAP_1, y = UMAP_2, color = val_subcluster)) +
  geom_point(size = 0.3, alpha = 0.8) +
  theme_bw(base_size = 11) +
  labs(title = "Velmeshev microglia reclustering")
ggsave(file.path(opt$outdir, "07_validation_microglia_umap_by_subcluster.pdf"), p_umap, width = 6.2, height = 5.2)
...
safe_fwrite(disc_pb_meta, file.path(opt$outdir, "08_discovery_paired_pseudobulk_manifest.tsv"))

log_msg("Running discovery paired state DE")
disc_state_de <- run_paired_state_de(
  disc_pb_counts,
  disc_pb_meta,
  file.path(opt$outdir, "09_discovery_state_definition_cluster2_vs_cluster0_paired"),
...
safe_fwrite(sig_info$table, file.path(opt$outdir, "10_discovery_frozen_state_signature.tsv"))
...
safe_fwrite(as.data.table(disc_centroids, keep.rownames = "gene"), file.path(opt$outdir, "11_discovery_centroids.tsv.gz"))

log_msg("Running validation state mapping")
val_data <- tryCatch(get_assay_mat(val_obj_mg, assay = val_assay, layer_name = "data"), error = function(e) NULL)
if (is.null(val_data) || nrow(val_data) == 0 || ncol(val_data) == 0) {
  val_obj_mg <- NormalizeData(val_obj_mg, assay = val_assay, verbose = FALSE)
  val_data <- get_assay_mat(val_obj_mg, assay = val_assay, layer_name = "data")
}
val_data <- val_data[, val_meta$cell, drop = FALSE]

val_centroids <- make_centroids(val_data, val_meta$val_subcluster)
safe_fwrite(as.data.table(val_centroids, keep.rownames = "gene"), file.path(opt$outdir, "12_validation_subcluster_centroids.tsv.gz"))
...
safe_fwrite(mapping_dt, file.path(opt$outdir, "13_validation_subcluster_mapping.tsv"))
...
log_msg("Candidate-like validation subclusters: %s", paste(candidate_clusters, collapse = ", "))
log_msg("Reference-like validation subclusters: %s", paste(reference_clusters, collapse = ", "))
...
if (nrow(mapping_dt) > 0) {
  p_map <- ggplot(mapping_dt, aes(x = corr_to_disc_cluster0, y = corr_to_disc_cluster2, label = validation_cluster)) +
    ...
    labs(x = "Correlation to discovery cluster 0",
         ...
         title = "Validation microglia subcluster mapping")
  ggsave(file.path(opt$outdir, "14_validation_subcluster_mapping_scatter.pdf"), p_map, width = 6.2, height = 5.2)
}

log_msg("Scoring validation microglia cells with frozen discovery signature")
val_scores <- score_signature_matrix(val_data, sig_info$pos, sig_info$neg)
val_scores[, cell := val_meta$cell]
val_scores <- cbind(val_meta[, .(cell, donor, dx, val_subcluster)], val_scores)
val_scores[, is_candidate_cluster := val_subcluster %in% candidate_clusters]
val_scores[, is_reference_cluster := val_subcluster %in% reference_clusters]
val_scores[, in_retained_mapped_space := is_candidate_cluster | is_reference_cluster]
safe_fwrite(val_scores, file.path(opt$outdir, "15_validation_cell_scores.tsv.gz"))
...
safe_fwrite(donor_candidate, file.path(opt$outdir, "16_validation_donor_scores_candidateclusters.tsv"))
...
safe_fwrite(donor_reference, file.path(opt$outdir, "17_validation_donor_scores_referenceclusters.tsv"))
...
safe_fwrite(donor_retained, file.path(opt$outdir, "18_validation_donor_scores_retainedspace.tsv"))
...
safe_fwrite(donor_shift, file.path(opt$outdir, "19_validation_donor_candidate_vs_reference_shift.tsv"))
...
safe_fwrite(donor_abund, file.path(opt$outdir, "20_validation_candidate_cluster_abundance.tsv"))
...
safe_fwrite(rep_stats, file.path(opt$outdir, "21_validation_donor_replication_stats.tsv"))

if (nrow(donor_candidate) > 0) {
  plot_metric_box(donor_candidate, "candidate_minus_reference",
                  "Candidate minus reference",
                  file.path(opt$outdir, "22_plot_candidateclusters_candidate_minus_reference.pdf"),
...
if (nrow(donor_reference) > 0) {
  plot_metric_box(donor_reference, "candidate_minus_reference",
                  "Candidate minus reference",
                  file.path(opt$outdir, "23_plot_referenceclusters_candidate_minus_reference.pdf"),
...
if (nrow(donor_retained) > 0) {
  plot_metric_box(donor_retained, "candidate_minus_reference",
                  "Candidate minus reference",
                  file.path(opt$outdir, "24_plot_retainedspace_candidate_minus_reference.pdf"),
...
if (nrow(donor_shift) > 0) {
  plot_metric_box(donor_shift, "shift_candidate_minus_reference",
                  ...
                  file.path(opt$outdir, "25_plot_candidate_vs_reference_shift.pdf"),
...
if (nrow(donor_abund) > 0) {
  plot_metric_box(donor_abund, "prop_candidate",
                  ...
                  file.path(opt$outdir, "26_plot_candidate_cluster_proportion.pdf"),
                  "Validation donor-level candidate abundance")
}

log_msg("Running validation pseudobulk replication")
val_counts_mg <- get_assay_mat(val_obj_mg, assay = val_assay, layer_name = "counts")
val_counts_mg <- val_counts_mg[, val_meta$cell, drop = FALSE]

val_meta_retained <- val_meta[val_subcluster %in% c(candidate_clusters, reference_clusters)]
if (nrow(val_meta_retained) > 0) {
  pb_ret <- aggregate_counts_sparse(val_counts_mg[, val_meta_retained$cell, drop = FALSE], val_meta_retained$donor)
  ...
  safe_fwrite(pb_ret_meta, file.path(opt$outdir, "27_validation_pseudobulk_manifest_retainedspace.tsv"))
  run_edger_case_control(pb_ret, pb_ret_meta,
                         file.path(opt$outdir, "28_validation_pseudobulk_retainedspace_ASD_vs_Control"),
                         case_label = opt$case_label, control_label = opt$control_label)
}

if (length(candidate_clusters) > 0) {
  val_meta_cand <- val_meta[val_subcluster %in% candidate_clusters]
  if (nrow(val_meta_cand) > 0) {
    pb_cand <- aggregate_counts_sparse(val_counts_mg[, val_meta_cand$cell, drop = FALSE], val_meta_cand$donor)
    ...
    safe_fwrite(pb_cand_meta, file.path(opt$outdir, "29_validation_pseudobulk_manifest_candidateclusters.tsv"))
    run_edger_case_control(pb_cand, pb_cand_meta,
                           file.path(opt$outdir, "30_validation_pseudobulk_candidateclusters_ASD_vs_Control"),
                           case_label = opt$case_label, control_label = opt$control_label)
  }
}

if (length(reference_clusters) > 0) {
  val_meta_ref <- val_meta[val_subcluster %in% reference_clusters]
  if (nrow(val_meta_ref) > 0) {
    pb_ref <- aggregate_counts_sparse(val_counts_mg[, val_meta_ref$cell, drop = FALSE], val_meta_ref$donor)
    ...
    safe_fwrite(pb_ref_meta, file.path(opt$outdir, "31_validation_pseudobulk_manifest_referenceclusters.tsv"))
    run_edger_case_control(pb_ref, pb_ref_meta,
                           file.path(opt$outdir, "32_validation_pseudobulk_referenceclusters_ASD_vs_Control"),
                           case_label = opt$case_label, control_label = opt$control_label)
  }
}

if (length(candidate_clusters) > 0 && length(reference_clusters) > 0) {
  val_meta_pair <- val_meta[val_subcluster %in% c(candidate_clusters, reference_clusters)]
  val_meta_pair[, group := ifelse(val_subcluster %in% candidate_clusters, "candidate", "reference")]
  ...
  pb_pair <- aggregate_counts_sparse(
    val_counts_mg[, val_meta_pair$cell, drop = FALSE],
    paste(val_meta_pair$donor, val_meta_pair$group, sep = "__")
  )
  pb_pair <- pb_pair[, val_pair_group$sample_id, drop = FALSE]
  safe_fwrite(val_pair_group, file.path(opt$outdir, "33_validation_pseudobulk_manifest_candidate_reference_pairs.tsv"))

  run_pseudobulk_donor_delta(
    pb_pair, val_pair_group,
    file.path(opt$outdir, "34_validation_pseudobulk_donorDelta_candidate_minus_reference_ASD_vs_Control"),
...
decision <- if (mapping_support && n_donor_sig >= 2) {
  "supported cross-cohort replication"
} else if (mapping_support && n_donor_sig >= 1) {
...
safe_fwrite(decision_dt, file.path(opt$outdir, "35_replication_decision.tsv"))

decision_lines <- c(
  sprintf("Replication decision: %s", decision),
  sprintf("Mapping support: %s", mapping_support),
  sprintf("Candidate-like validation subclusters: %s", paste(candidate_clusters, collapse = ", ")),
  sprintf("Reference-like validation subclusters: %s", paste(reference_clusters, collapse = ", ")),
  sprintf("Number of donor-level supported metrics: %s", n_donor_sig),
  "Stopping rule:",
  "- If cluster mapping is weak and donor scores are not supportive -> limited cross-cohort replication.",
  "- If mapping is present and at least one donor-aware feature is directionally supportive -> partial cross-cohort replication.",
  "- If mapping is present and multiple donor-aware features are directionally supportive -> supported cross-cohort replication."
)
writeLines(decision_lines, con = file.path(opt$outdir, "36_replication_decision_notes.txt"))

capture.output(sessionInfo(), file = file.path(opt$outdir, "37_sessionInfo.txt"))
