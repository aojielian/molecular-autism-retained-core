...
option_list <- list(
  ...
  make_option("--global_rds", type = "character", default = Sys.getenv("MA_GLOBAL_RDS", NA_character_),
              help = "Optional path to global cortical Seurat object (.rds) [default: unset]"),
  make_option("--outdir", type = "character",
              default = file.path(DEFAULT_PROJECT_DIR, "results", "Package2_Define_Cluster2")),
  make_option("--assay", type = "character", default = "RNA"),
  make_option("--strict_cluster_col", type = "character", default = NA_character_),
  make_option("--strict_donor_col", type = "character", default = NA_character_),
  make_option("--strict_dx_col", type = "character", default = NA_character_),
  make_option("--global_celltype_col", type = "character", default = "annotation"),
  ...
  make_option("--top_n_core", type = "integer", default = 50),
  make_option("--top_n_global_audit", type = "integer", default = 15),
  make_option("--optional_gmt_files", type = "character", default = "",
              help = "Comma-separated GMT file paths [optional]")
)
...
cfg <- list(
  ...
  global_rds = if (is.na(opt$global_rds) || opt$global_rds == "") NA_character_ else opt$global_rds,
  outdir = opt$outdir,
  assay = opt$assay,
  strict_cluster_col = if (is.na(opt$strict_cluster_col) || opt$strict_cluster_col == "") NULL else opt$strict_cluster_col,
  strict_donor_col = if (is.na(opt$strict_donor_col) || opt$strict_donor_col == "") NULL else opt$strict_donor_col,
  strict_dx_col = if (is.na(opt$strict_dx_col) || opt$strict_dx_col == "") NULL else opt$strict_dx_col,
  global_celltype_col = opt$global_celltype_col,
  ref_cluster = opt$ref_cluster,
  cand_cluster = opt$cand_cluster,
  min_cells_per_pb_sample = opt$min_cells_per_pb_sample,
  top_n_core = opt$top_n_core,
  top_n_global_audit = opt$top_n_global_audit,
  audit_genes = c("SPP1","GAS6","APOE","MERTK","CD74","C3"),
  homeostatic_genes = c("P2RY12","CX3CR1","TMEM119","CSF1R","AIF1","C1QA","C1QB","C1QC"),
  optional_gmt_files = if (opt$optional_gmt_files == "") character(0) else strsplit(opt$optional_gmt_files, ",")[[1]]
)
