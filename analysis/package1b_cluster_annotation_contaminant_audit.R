...
option_list <- list(
  ...
  make_option("--outdir", type = "character",
              default = file.path(DEFAULT_PROJECT_DIR, "results", "Package1b_ClusterAnnotation_ContaminantAudit"),
  ...
  make_option("--marker_file", type = "character", default = NA_character_),
  make_option("--top_n_markers", type = "integer", default = 50),
  make_option("--min_pct", type = "double", default = 0.10),
  make_option("--logfc", type = "double", default = 0.25),
  make_option("--delta_strict", type = "double", default = 0.35),
  make_option("--delta_mixed", type = "double", default = 0.15),
  make_option("--min_mg_anchor_hits_strict", type = "integer", default = 2),
  make_option("--min_nonmg_hits_contam", type = "integer", default = 2)
)
...
cfg <- list(
  ...
  marker_file    = if (is.na(opt$marker_file) || opt$marker_file == "") NA_character_ else opt$marker_file,
  top_n_markers  = opt$top_n_markers,
  min_pct        = opt$min_pct,
  logfc          = opt$logfc,
  delta_strict   = opt$delta_strict,
  delta_mixed    = opt$delta_mixed,
  min_mg_anchor_hits_strict = opt$min_mg_anchor_hits_strict,
  min_nonmg_hits_contam     = opt$min_nonmg_hits_contam
)
