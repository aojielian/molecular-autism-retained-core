...
if (length(args) >= 2) {
  ...
  infile <- Sys.getenv("MA_GANDAL_SCORE_TSV", unset = "")
  outdir <- Sys.getenv("MA_GANDAL_LOO_OUTDIR", unset = "")

  if (infile == "" || outdir == "") {
    stop(paste(
      "Usage: Rscript Step08b_Gandal2022_leave_one_region_out_sensitivity_github_ready.R <sample_level_scores.tsv> <outdir>",
      sprintf("or set environment variables MA_GANDAL_SCORE_TSV and MA_GANDAL_LOO_OUTDIR"),
      sep = "
"
    ))
  }
