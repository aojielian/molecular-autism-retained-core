...
option_list <- list(
  make_option("--disc_rds", type = "character",
              default = Sys.getenv("MA_STRICT_RDS", NA_character_)),
  make_option("--val_rds", type = "character",
              default = Sys.getenv("MA_VELMESH_RDS", NA_character_)),
  make_option("--outdir", type = "character",
              default = file.path(DEFAULT_PROJECT_DIR, "results", "Package4_CrossCohortValidation_Velmeshev_v2")),
...
if (is.na(opt$disc_rds) || opt$disc_rds == "") stop("Please provide --disc_rds or set MA_STRICT_RDS.")
if (is.na(opt$val_rds) || opt$val_rds == "") stop("Please provide --val_rds or set MA_VELMESH_RDS.")
