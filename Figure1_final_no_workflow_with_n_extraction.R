...
# -------------------------------------------------
# Extract exact n values for Figure 1
# -------------------------------------------------
msg("Extracting exact n values for Figure 1 ...")

disc_meta_n <- as.data.table(disc_obj@meta.data, keep.rownames = "cell")
disc_meta_n[, donor_use := as.character(get(disc_donor_col))]
disc_meta_n[, dx_use := normalize_dx(get(disc_dx_col))]
disc_meta_n[, cluster_use := as.character(disc_obj$cluster_use)]

disc_n_total_cells  <- ncol(disc_obj)
disc_n_total_donors <- uniqueN(disc_meta_n[!is.na(donor_use) & donor_use != "", donor_use])
disc_n_clusters     <- length(cluster_levels)

disc_dx_summary <- disc_meta_n[
  dx_use %in% c("Control", "ASD") & !is.na(donor_use) & donor_use != "",
  .(n_cells = .N, n_donors = uniqueN(donor_use)),
  by = dx_use
][order(factor(dx_use, levels = c("Control", "ASD")))]

strict_meta_n <- as.data.table(strict_obj@meta.data, keep.rownames = "cell")
strict_meta_n[, donor_use := as.character(get(strict_donor_col))]
strict_meta_n[, dx_use := normalize_dx(get(strict_dx_col))]
strict_meta_n[, cluster_use := as.character(strict_obj$cluster_use)]

strict_cluster_levels <- make_cluster_levels(strict_meta_n$cluster_use)
strict_n_total_cells  <- ncol(strict_obj)
strict_n_total_donors <- uniqueN(strict_meta_n[!is.na(donor_use) & donor_use != "", donor_use])
strict_n_clusters     <- length(strict_cluster_levels)

strict_dx_summary <- strict_meta_n[
  dx_use %in% c("Control", "ASD") & !is.na(donor_use) & donor_use != "",
  .(n_cells = .N, n_donors = uniqueN(donor_use)),
  by = dx_use
][order(factor(dx_use, levels = c("Control", "ASD")))]

fig1_exact_n <- rbindlist(list(
  data.table(
    panel = "A/B",
    object = "discovery_stage_object",
    diagnosis = "All",
    n_cells = disc_n_total_cells,
    n_donors = disc_n_total_donors,
    n_clusters = disc_n_clusters
  ),
  disc_dx_summary[, .(
    panel = "A/B",
    object = "discovery_stage_object",
  ...
  data.table(
    panel = "D/E",
    object = "strict_retained_core",
    diagnosis = "All",
    n_cells = strict_n_total_cells,
    n_donors = strict_n_total_donors,
    n_clusters = strict_n_clusters
  ),
  strict_dx_summary[, .(
    panel = "D/E",
    object = "strict_retained_core",
...
safe_write(fig1_exact_n, file.path(cfg$outdir, "tables", "00_Figure1_exact_n.tsv"))

fig1_summary_lines <- c(
  "Figure 1 exact n summary",
  sprintf("Panels A/B (discovery object): %s cells, %s donors, %s clusters", disc_n_total_cells, disc_n_total_donors, disc_n_clusters),
  if (nrow(disc_dx_summary) > 0) sprintf("Panels A/B %s: %s cells, %s donors", disc_dx_summary$dx_use, disc_dx_summary$n_cells, disc_dx_summary$n_donors),
  sprintf("Panels D/E (strict retained core): %s cells, %s donors, %s retained clusters", strict_n_total_cells, strict_n_total_donors, strict_n_clusters),
  if (nrow(strict_dx_summary) > 0) sprintf("Panels D/E %s: %s cells, %s donors", strict_dx_summary$dx_use, strict_dx_summary$n_cells, strict_dx_summary$n_donors)
)
writeLines(fig1_summary_lines, con = file.path(cfg$outdir, "tables", "00_Figure1_exact_n_summary.txt"))
