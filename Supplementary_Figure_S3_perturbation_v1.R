...
root <- "/Users/aojie/PROJECTs/molecular_autism/drug/Package9_PerturbationPrioritization_local"
outdir <- file.path(root, "supplementary_figure_S3")
...
top_n_primary <- 12
top_n_sens <- 12
primary_top <- primary[order(-reversal_score)][1:min(top_n_primary, .N)]
sens_top    <- sens[order(-reversal_score)][1:min(top_n_sens, .N)]

primary_top[, label := factor(label, levels = rev(label))]
sens_top[, label := factor(label, levels = rev(label))]

# Representative labels for scatter
label_hits <- unique(c(
...
muted_primary <- "#C97B84"
muted_sens <- "#B98E6C"
muted_point <- "#BFBFBF"

pA <- ggplot(primary_top, aes(x = reversal_score, y = label)) +
  geom_col(fill = muted_primary, width = 0.72) +
...
pB <- ggplot(primary_scatter, aes(x = neglog10_adjP_down, y = neglog10_adjP_up)) +
  geom_point(color = muted_point, alpha = 0.8, size = 1.8) +
  geom_point(
    data = primary_scatter[annotate_hit == TRUE],
    color = muted_primary, size = 2.1
  ) +
  geom_text_repel(
    data = primary_scatter[annotate_hit == TRUE],
...
pC <- ggplot(sens_top, aes(x = reversal_score, y = label)) +
  geom_col(fill = muted_sens, width = 0.72) +
  geom_text(aes(label = sprintf("%.2f", reversal_score)),
            hjust = -0.1, size = 3.2) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.14))) +
  labs(
...
# also export concise support tables for manuscript assembly
