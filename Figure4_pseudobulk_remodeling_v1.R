...
cfg <- list(
  ...
  outdir = "/gpfs/hpc/home/lijc/lianaoj/autism_scRNA/MolecularAutism_Revision/Figure4_Pseudobulk_Remodeling_v1",
...
choose_labels <- function(dt, must_label = character(), max_labels = 10) {
  dt <- copy(dt)
...
add_labels <- function(p, lab_dt, size = 3.2) {
...
plot_volcano <- function(dt, title, must_label) {
  lab_dt <- choose_labels(dt, must_label = must_label, max_labels = cfg$max_labels)
  ...
  stats <- make_stats(dt)
  ann <- paste0(
    "FDR < 0.10 ASD-up: ", stats$n_fdr10_up,
    "\nFDR < 0.10 Control-up: ", stats$n_fdr10_down,
...
# =========================================================
# Label sets from manuscript narrative
# =========================================================
labels_pooled <- c("SPP1", "FCGR2A", "ACSL1", "HIF1A", "P4HA1", "SOAT1", "HSPA1A", "HSPA1B", "MYO1E")
labels_cluster0 <- c("SPP1", "CHST15", "ZC3HAV1", "P4HA1", "STAT3", "HIF1A", "LY96", "REL", "SCIN")
labels_cluster2 <- c("SPP1", "B2M", "GNA13", "HSPA1A", "ASAH1", "MYO1E", "SKAP2", "P4HA1")
...
pC <- plot_volcano(
  cluster2,
  title = "C. Cluster 2 carries remodeling-associated ASD signal",
